	...
	function getParseString($str) {
		...
		for ($x=1; $x<sizeof($arr); $x++) {
			...
			for ($y=1; $y<sizeof($arr2); $y++) {
				...
				switch (trim($mas[0])) {
					case "DTSTART":
						$arr_n[$x]['start_date'] = $this->getMySQLDate(trim($mas[1]));
						break;

					case "DTEND":
						$arr_n[$x]['end_date'] = $this->getMySQLDate(trim($mas[1]));
						break;

					case "RRULE":
						//echo'<pre>';var_dump($mas,$mas_);echo'</pre>';
						$rrule = explode(";", trim($mas[1]));
